...
class SetupSOSScreen extends StatefulWidget {
  ...
  @override State<SetupSOSScreen> createState() => _SetupSOSScreenState();
}
...
class _SetupSOSScreenState extends State<SetupSOSScreen> {
  ...
  void initState() {
    super.initState();
    final s = AppState();
    _policeCtrl  = TextEditingController(text: s.policeStation.isEmpty ? 'Maharagama' : s.policeStation);
    _hospitalCtrl = TextEditingController(text: s.hospital.isEmpty ? 'Kalubowila' : s.hospital);
    _locCtrl = TextEditingController(text: s.locationLabel.isEmpty ? '' : s.locationLabel);
    _locationSet = s.hasLocation;
  }

  @override void dispose() {
    _policeCtrl.dispose(); _hospitalCtrl.dispose(); _locCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _pickLocation() async {
    ...
    if (result != null) {
      setState(() { _locCtrl.text = result; _locationSet = true; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Setup SOS', style: GoogleFonts.inter(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Expanded(child: Text(
                ...
                style: GoogleFonts.inter(fontSize: 12,
                    color: const Color(0xFFE53935), height: 1.5))),
            ])),
          const SizedBox(height: 16),

          Text('We need to know your location in order to suggest nearby help',
            style: GoogleFonts.inter(fontSize: 14,
                color: const Color(0xFF1C1C1E), height: 1.5)),
            ...
            child: Container(
              ...
              child: ClipRRect(
                ...
                child: Stack(alignment: Alignment.center, children: [
                  CustomPaint(
                    size: Size(MediaQuery.of(context).size.width-40, 170),
                    painter: _MiniMap()),
                  _Radar(),
                  if (!_locationSet)
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.touch_app, color: Colors.white, size: 15),
                        const SizedBox(width: 6),
                        Text('Tap to pick location', style: GoogleFonts.inter(
          ...
          if (_locationSet) ...[
            Container(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 10),
                Expanded(child: Text(_locCtrl.text,
          ...
          SizedBox(width: double.infinity, height: 50,
            child: ElevatedButton.icon(
              ...
              icon: const Icon(Icons.my_location_rounded, size: 18,
                  color: Colors.white),
              label: Text('Use Current Location', style: GoogleFonts.inter(
                  fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFFFFC107), elevation: 0,
          ...
          SizedBox(width: double.infinity, height: 54,
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                ...
                shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(30))),
              child: Text('Setup SOS', style: GoogleFonts.inter(
...
class _MiniMap extends CustomPainter {
  @override void paint(Canvas canvas, Size size) {
    ...
    final rp=Paint()..color=Colors.white.withOpacity(0.7)..strokeWidth=1;
    ...
    for (final b in [
      Rect.fromLTWH(8,8,55,30),Rect.fromLTWH(72,12,45,42),
      Rect.fromLTWH(126,4,62,26),Rect.fromLTWH(8,50,52,38),
      Rect.fromLTWH(126,48,58,40),Rect.fromLTWH(200,20,48,34),
    ]) canvas.drawRRect(RRect.fromRectAndRadius(b,const Radius.circular(3)),bp);
  }
...
class _Radar extends StatefulWidget {
  const _Radar();
  @override State<_Radar> createState() => _RadarState();
}
class _RadarState extends State<_Radar> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) => AnimatedBuilder(animation:_a,
    builder:(_,__)=>Stack(alignment:Alignment.center,children:[
      Container(width:80*_a.value,height:80*_a.value,
        decoration:BoxDecoration(shape:BoxShape.circle,
          color:const Color(0xFFE53935).withOpacity((1-_a.value)*0.3))),
      Container(width:40*_a.value,height:40*_a.value,
        decoration:BoxDecoration(shape:BoxShape.circle,
          color:const Color(0xFFE53935).withOpacity((1-_a.value)*0.2))),
      Container(width:14,height:14,
        decoration:const BoxDecoration(shape:BoxShape.circle,
          color:Color(0xFFE53935),
          boxShadow:[BoxShadow(color:Color(0x88E53935),blurRadius:8,spreadRadius:2)])),
    ]));
}
